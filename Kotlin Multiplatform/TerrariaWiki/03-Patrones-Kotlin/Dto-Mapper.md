...
- **Listas con `^`.** El campo `type` puede ser `"block^crafting material"`.
- **`sell` viene con HTML** (`<span class="coin" title="60 Copper Coins" data-sort-value="60">`). Limpiamos con `stripHtml()` y extraemos el numérico con regex `data-sort-value="(\d+)"`.
- **Stats vacías como `""`** (no `null`). `toIntOrNull() ?: null` lo deja en `null` cuando están vacías.
- **Imágenes en `image`** como wikitext: `[[File:Wood.png|...]]`. Extraemos con regex `\[\[File:([^\]|]+)`.
